...
export async function POST(request: NextRequest) {
  ...
  if (!barcode && !manualUrl) {
    return err(
      "Provide a barcode or a product page URL.",
      "BAD_REQUEST",
      400
    );
  }
  ...
  try {
    ...
    if (e instanceof PageBlockedError) {
      return err(
        ...
        "PAGE_BLOCKED",
        422,
        "The page blocked automated access. Try a different URL or paste the page content manually."
      );
    }
    if (e instanceof PageEmptyError) {
      return err(
        ...
        "PAGE_EMPTY",
        422,
        "The page requires JavaScript to render. Try a different URL or paste the page content manually."
      );
    }
    ...
    if (e instanceof PageTimeoutError) {
      return err(e.message, "PAGE_TIMEOUT", 504, "The page took too long to respond. Try again or use a different URL.");
    }
